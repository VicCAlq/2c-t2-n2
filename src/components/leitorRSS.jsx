...
async function fetchComProxy(endereco) {
  ...
  throw (
    erroMaisRecente ||
    new Error(
      "Não foi possível carregar o feed. Verifique a Endereco e tente novamente.",
    )
  );
}
...
function lerRSS(textoXML) {
  ...
  if (formatoAtom) {
    ...
    noticias = Array.from(valores).map((valor) => {
      const valorLink =
        valor.querySelector('link[rel="alternate"]')?.getAttribute("href") ||
      ...
      return {
        ...
        dataPublicacao:
          valor.querySelector("updated")?.textContent ||
          valor.querySelector("published")?.textContent ||
          new Date().toISOString(),
        categorias: Array.from(valor.querySelectorAll("category"))
    ...
    noticias = Array.from(elementosnoticias).map((item) => {
      ...
      return {
        ...
        dataPublicacao:
          ...
          new Date().toISOString(),
        categorias: Array.from(item.querySelectorAll("category"))
          .map((c) => c.textContent)
          .filter(Boolean),
      };
    });
